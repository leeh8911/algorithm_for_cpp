...
TEST(DividablePackingTest, SampleCase) {
    ...
    std::vector<int64_t> prices = {10, 7, 15, 10, 12, 11, 5};
    ...
    for (; weight_iter != weights.end(); ++weight_iter, ++price_iter) {
        objects.emplace_back(*id_iter, *weight_iter, *price_iter);
    }

    auto result = algorithm::DividablePacking(objects);
    ...
    EXPECT_EQ(algorithm::Object{2, 2, 7}, result[3]);
}